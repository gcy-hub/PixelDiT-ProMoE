...
class LightningModel(pl.LightningModule):
    def __init__(self,
        ...
        self.enable_ema = bool(enable_ema)
        self.compile_model = bool(compile_model)
        self.ema_denoiser = copy.deepcopy(self.denoiser) if self.enable_ema else None
    ...
    def configure_callbacks(self) -> Union[Sequence[Callback], Callback]:
        return [self.ema_tracker] if self.enable_ema and self.ema_tracker is not None else []
    ...
    def on_train_start(self) -> None:
        if self.ema_denoiser is not None:
            self.ema_denoiser.to(torch.float32)
        if self.ema_denoiser is not None and self.ema_tracker is not None:
            self.ema_tracker.setup_models(net=self.denoiser, ema_net=self.ema_denoiser)
        if self.global_rank == 0 and hasattr(self.denoiser, "model_configuration"):
            config = self.denoiser.model_configuration()
            print(
                "[model] "
                f"{config['architecture']} E{config['total_experts_per_promoe_block']}A1S1U1: "
                f"total={config['total_parameters']:,}, "
                f"activated/conditional={config['activated_parameters_per_conditional_token']:,}, "
                f"activated/null={config['activated_parameters_per_null_token']:,}; "
                f"patch={config['patch_blocks']} (ProMoE={config['promoe_patch_blocks']}), "
                f"PiT={config['pit_blocks']}, expert_intermediate={config['expert_intermediate_size']}",
                flush=True,
            )
    ...
    def predict_step(self, batch, batch_idx):
        ...
        if self.eval_original_model or self.ema_denoiser is None:

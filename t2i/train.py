...
from diffusion.ocr import UniRecWholeImageLoss
from diffusion.model.respace import compute_density_for_timestep_sampling
from diffusion.model.utils import get_weight_dtype
from diffusion.utils.checkpoint import load_checkpoint, prune_recovery_checkpoints, save_checkpoint
...
def load_validation_prompts(config) -> list[str]:
    """Load held-out validation prompts when a manifest is configured."""
    manifest_path = getattr(config.train, "validation_prompts_file", None)
    if not manifest_path:
        return list(config.train.validation_prompts)
    path = Path(manifest_path).expanduser()
    if not path.is_file():
        raise FileNotFoundError(f"Validation prompt manifest not found: {path}")
    try:
        manifest = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        raise ValueError(f"Validation prompt manifest is not valid JSON: {path}") from error
    samples = manifest.get("samples") if isinstance(manifest, dict) else None
    if not isinstance(samples, list) or not samples:
        raise ValueError(f"Validation prompt manifest has no non-empty samples list: {path}")
    prompts = [str(sample.get("prompt", "")).strip() for sample in samples if isinstance(sample, dict)]
    if len(prompts) != len(samples) or not all(prompts):
        raise ValueError(f"Every validation sample must have a non-empty prompt: {path}")
    return prompts


def validation_prompt_embed_path(prompt: str) -> str:
    """Return a filesystem-safe, collision-resistant validation cache name."""
    prompt_hash = hashlib.sha256(prompt.encode("utf-8")).hexdigest()
    return f"{prompt_hash}_{valid_prompt_embed_suffix}"
...
def log_validation(accelerator, config, model, logger, step, device, init_noise=None):
    ...
    def run_sampling(init_z=None, label_suffix="", sampler="flow_dpm-solver"):
        ...
        for prompt in validation_prompts:
            ...
            embed = torch.load(
                osp.join(config.train.valid_prompt_embed_root, validation_prompt_embed_path(prompt)),
                map_location="cpu",
            )
...
def train(
    ...
    for epoch in range(start_epoch + 1, config.train.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # Keep this one-dimensional mask: OCR supervision only applies to
            # samples that retained their text condition.
            condition_dropped = torch.zeros(bs, dtype=torch.bool, device=clean_images.device)
            ...
            if p_drop > 0 and isinstance(y, torch.Tensor):
                ...
                condition_dropped = drop_mask.flatten()
            ...
            with accelerator.accumulate(model):
                ...
                loss_term = train_diffusion.training_losses(
                    ...
                    return_pred_xstart=ocr_loss_model is not None,
                )
                flow_loss = loss_term["loss"].mean()
                loss = flow_loss
                ocr_loss = clean_images.new_zeros(())
                ocr_samples = clean_images.new_zeros((), dtype=torch.long)

                if ocr_loss_model is not None:
                    if len(batch) < 9:
                        raise RuntimeError(
                            "OCR loss is enabled but the dataset did not return transcripts. "
                            "Set data.return_ocr_transcript: true."
                        )
                    sigma_values = torch.as_tensor(train_diffusion.sigmas, device=clean_images.device)[timesteps]
                    ocr_mask = (sigma_values <= float(config.train.ocr_max_sigma)) & ~condition_dropped
                    if float(config.train.ocr_probability) < 1.0:
                        ocr_mask &= torch.rand(bs, device=clean_images.device) < float(config.train.ocr_probability)
                    if torch.any(ocr_mask):
                        selected_indices = torch.nonzero(ocr_mask, as_tuple=False).flatten()
                        max_ocr_samples = int(getattr(config.train, "ocr_max_samples_per_batch", 1))
                        if max_ocr_samples <= 0:
                            raise ValueError("train.ocr_max_samples_per_batch must be positive when OCR loss is enabled.")
                        if selected_indices.numel() > max_ocr_samples:
                            selected_indices = selected_indices[torch.randperm(
                                selected_indices.numel(), device=selected_indices.device
                            )[:max_ocr_samples]]
                        selected_transcripts = [batch[8][int(index)] for index in selected_indices.tolist()]
                        ocr_loss, ocr_count = ocr_loss_model(
                            loss_term["pred_xstart"].index_select(0, selected_indices), selected_transcripts
                        )
                        ocr_samples = torch.as_tensor(ocr_count, device=clean_images.device, dtype=torch.long)
                        if ocr_count:
                            loss = loss + float(config.train.ocr_loss_weight) * ocr_loss
            ...
            logs = {
                ...
                "flow_loss": accelerator.gather(flow_loss).mean().item(),
            }
            if ocr_loss_model is not None:
                logs.update(
                    ocr_loss=accelerator.gather(ocr_loss.detach()).mean().item(),
                    ocr_samples=accelerator.gather(ocr_samples).sum().item(),
                )
            ...
            if (
                ...
                if accelerator.is_main_process:
                    prune_recovery_checkpoints(
                        osp.join(config.work_dir, "checkpoints"), config.train.keep_recovery_checkpoints
                    )
            ...
            if config.train.visualize and (
                ...
                or (config.train.validate_at_start and (step + 1) == 1)
            ):
                if config.train.use_fsdp:
            ...
            if config.train.max_train_steps is not None and global_step >= config.train.max_train_steps:
                logger.info(f"Stopping training at configured max_train_steps={config.train.max_train_steps}.")
                return
        ...
        # A zero epoch interval disables milestone checkpoints. Recovery
        # checkpoints are handled independently by save_model_steps above.
        if (
            config.train.save_model_epochs > 0
            and (epoch % config.train.save_model_epochs == 0 or epoch == config.train.num_epochs)
...
def main(cfg: PixDiTConfig) -> None:
    ...
    config.train.validation_prompts = load_validation_prompts(config)

    if config.learning_rate is not None:
        config.train.optimizer["lr"] = float(config.learning_rate)
    ...
    if config.train.visualize and len(config.train.validation_prompts):
        ...
        for prompt in validation_prompts:
            prompt_embed_path = osp.join(config.train.valid_prompt_embed_root, validation_prompt_embed_path(prompt))
        ...
        if accelerator.is_main_process and not skip:
            ...
            for prompt in validation_prompts:
                prompt_embed_path = osp.join(
                    config.train.valid_prompt_embed_root, validation_prompt_embed_path(prompt)
                )
                if (
    ...
    if config.model.multi_scale:
        ...
        configured_cache_root = os.environ.get("WIDS_BATCHSAMPLER_CACHE")
        if configured_cache_root:
            cache_root = osp.expanduser(configured_cache_root)
        else:
    ...
    ocr_loss_model = None
    ocr_weight = float(getattr(config.train, "ocr_loss_weight", 0.0))
    if ocr_weight > 0.0:
        if not config.data.return_ocr_transcript:
            raise ValueError("OCR loss requires data.return_ocr_transcript: true.")
        if not config.train.ocr_model_path:
            raise ValueError("OCR loss requires train.ocr_model_path to point to local UniRec weights.")
        if not 0.0 < float(config.train.ocr_probability) <= 1.0:
            raise ValueError("train.ocr_probability must be in (0, 1].")
        if not 0.0 < float(config.train.ocr_max_sigma) <= 1.0:
            raise ValueError("train.ocr_max_sigma must be in (0, 1].")
        if int(getattr(config.train, "ocr_max_samples_per_batch", 1)) <= 0:
            raise ValueError("train.ocr_max_samples_per_batch must be positive.")
        ocr_dtype = torch.bfloat16 if config.model.mixed_precision == "bf16" else torch.float32
        ocr_loss_model = UniRecWholeImageLoss(
            model_path=config.train.ocr_model_path,
            max_tokens=config.train.ocr_max_tokens,
            max_width=config.train.ocr_max_width,
            max_height=config.train.ocr_max_height,
            size_divisor=config.train.ocr_size_divisor,
            dtype=ocr_dtype,
        ).to(accelerator.device, dtype=ocr_dtype)
        ocr_loss_model.eval()
        logger.info(
            "Enabled frozen whole-image UniRec OCR loss: "
            f"weight={ocr_weight}, probability={config.train.ocr_probability}, "
            f"max_sigma={config.train.ocr_max_sigma}, model={config.train.ocr_model_path}"
        )

...
class GaussianDiffusion:
    ...
    def training_losses(
        ...
        """
        # ``t`` can be expanded below by the legacy SNR-loss branch. Keep the
        # original one-dimensional timestep tensor for x_0 reconstruction.
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if return_pred_xstart:
                if self.model_mean_type == ModelMeanType.FLOW_VELOCITY:
                    # Flow matching uses x_t = x_0 + sigma_t * v.  Keep this
                    # tensor attached to the graph for optional image losses.
                    sigma = _extract_into_tensor(self.sigmas, timestep, x_t.shape)
                    terms["pred_xstart"] = x_t - sigma * output
                elif self.model_mean_type == ModelMeanType.START_X:
                    terms["pred_xstart"] = output
                elif self.model_mean_type == ModelMeanType.EPSILON:
                    terms["pred_xstart"] = self._predict_xstart_from_eps(x_t=x_t, t=t, eps=output)
                else:
                    raise NotImplementedError(
                        f"pred_xstart is not implemented for {self.model_mean_type}."
                    )

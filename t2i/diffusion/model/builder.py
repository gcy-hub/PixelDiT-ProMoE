...
def get_tokenizer_and_text_encoder(name="gemma-2-2b-it", device="cuda"):
    ...
    if "gemma" in name or "Qwen" in name:
        model_source = text_encoder_dict[name]
        load_kwargs = {}
        if "gemma" in name:
            model_source = os.environ.get("PIXDIT_GEMMA_PATH", "")
            if not model_source:
                raise EnvironmentError(
                    "PIXDIT_GEMMA_PATH is required for Gemma. PixelDiT will not download Gemma from Hugging Face."
                )
            if not os.path.isdir(model_source):
                raise FileNotFoundError(f"PIXDIT_GEMMA_PATH is not a directory: {model_source}")
            load_kwargs["local_files_only"] = True
        tokenizer = AutoTokenizer.from_pretrained(model_source, **load_kwargs)
        tokenizer.padding_side = "right"
        print(f"loading text encoder from {model_source}")
        text_encoder = (
            AutoModelForCausalLM.from_pretrained(model_source, torch_dtype=torch.bfloat16, **load_kwargs)
            .get_decoder()
            .to(device)
        )
    else:

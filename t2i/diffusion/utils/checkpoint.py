...
def prune_recovery_checkpoints(work_dir, keep_last):
    """Keep only the newest step recovery checkpoints, never epoch milestones."""
    if keep_last < 1:
        raise ValueError("keep_last must be at least one")
    pattern = re.compile(r"^epoch_(\d+)_step_(\d+)_recovery(?:\.pth)?$")
    candidates = {}
    for name in os.listdir(work_dir):
        match = pattern.match(name)
        if match and not os.path.islink(os.path.join(work_dir, name)):
            checkpoint_id = (int(match.group(2)), int(match.group(1)))
            candidates.setdefault(checkpoint_id, []).append(name)
    for _, names in sorted(candidates.items())[:-keep_last]:
        for name in names:
...
def load_checkpoint_ddp(
    ...
    checkpoint = _load_model(ckpt_file)
...
def load_checkpoint_fsdp(
    ...
    if ".pth" in checkpoint:
        state_dict_model = _load_model(checkpoint)
        ...
        state_dict_model = _load_model(os.path.join(checkpoint, "model", "pytorch_model_fsdp.bin"))

...
class DataConfig(BaseConfig):
    ...
    hq_only: bool = False
    valid_num: int = 0
    # Keep OCR targets separate from the T2I prompt.  These fields only control
    # the transcript returned by text-annotated WIDS datasets; polygons are not
    # passed to the OCR loss.
    return_ocr_transcript: bool = False
    ocr_valid_only: bool = True
    ocr_min_rec_score: float = 0.0
    ocr_transcript_separator: str = "\n"
    data: Any = None
    extra: Any = None
...
class TrainingConfig(BaseConfig):
    ...
    keep_recovery_checkpoints: int = 1
    ...
    # Optional JSON manifest with a ``samples`` list. Each sample must contain
    # a non-empty ``prompt``. This keeps held-out validation prompts separate
    # from the training WIDS shards.
    validation_prompts_file: Optional[str] = None
    ...
    # Optional whole-image OCR supervision.  The OCR model is frozen; its
    # teacher-forced token loss still backpropagates through predicted x_0.
    ocr_loss_weight: float = 0.0
    ocr_model_path: Optional[str] = None
    ocr_probability: float = 0.25
    ocr_max_samples_per_batch: int = 1
    ocr_max_sigma: float = 0.25
    ocr_max_tokens: int = 256
    ocr_max_width: int = 960
    ocr_max_height: int = 1408
    ocr_size_divisor: int = 64
...
class PixDiTConfig(BaseConfig):
    ...
    work_dir: str = "output/"
    # Use empty strings for CLI compatibility with pyrallis' optional-string parser.
    resume_from: str = ""
    load_from: str = ""
    debug: bool = False
    caching: bool = False
    ...
    # Optional scalar override used by the portable launcher. Keeping it
    # outside the optimizer dict makes CLI parsing type-safe.

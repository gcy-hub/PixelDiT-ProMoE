...
def _pixel_interpolation(aspect_ratio):
    high_resolution_ratios = [
        value for value in (globals().get("ASPECT_RATIO_2048"), globals().get("ASPECT_RATIO_2880")) if value is not None
    ]
    return InterpolationMode.LANCZOS if aspect_ratio in high_resolution_ratios else InterpolationMode.BICUBIC


def _annotation_reading_key(polygon: Any) -> Tuple[float, float]:
    """Return a stable top-to-bottom, left-to-right key for an annotation."""
    try:
...
def _whole_image_ocr_transcript(
    info: Dict[str, Any],
    valid_only: bool,
    min_rec_score: float,
    separator: str,
) -> str:
    """Serialize text annotations without exposing their locations to OCR loss.

    The polygons are used only to impose a deterministic reading order on
    ground-truth strings.  The OCR model always receives the full predicted
    image, so this metadata cannot force generated text to a GT location.
    """
    entries: List[Tuple[Tuple[float, float], str]] = []
    annotations = info.get("annotations")
    has_structured_labels = False
    if isinstance(annotations, list):
        has_structured_labels = bool(annotations)
        for annotation in annotations:
            if not isinstance(annotation, dict):
                continue
            if valid_only and not bool(annotation.get("valid", True)):
                continue
            if float(annotation.get("rec_score", 1.0)) < min_rec_score:
                continue
            text = str(annotation.get("text", "")).strip()
            if text:
                entries.append((_annotation_reading_key(annotation.get("polygon")), text))
    else:
        ocr_result = info.get("ocr_result", [])
        has_structured_labels = isinstance(ocr_result, list) and bool(ocr_result)
        for item in ocr_result:
            try:
                ...
                continue
            if float(score) < min_rec_score:
                continue
            text = str(text).strip()
            if text:
                entries.append((_annotation_reading_key(polygon), text))

    if entries:
        entries.sort(key=lambda item: item[0])
        return separator.join(text for _, text in entries)

    # Some compatible WIDS datasets provide only text labels. Their stored
    # order is already the source reading order; do not invent coordinates.
    # Do not fall back to ``texts`` when OCR annotations were present but were
    # rejected by confidence/validity filtering: that would silently defeat
    # the configured quality threshold.
    if has_structured_labels:
        return ""
    texts = info.get("texts", [])
    if isinstance(texts, list):
        return separator.join(str(text).strip() for text in texts if str(text).strip())
    return ""
...
class PixDiTWebDataset(Dataset):
    ...
    def __init__(
        ...
        clip_thr=0.0,
        clip_thr_temperature=1.0,
        return_ocr_transcript=False,
        ocr_valid_only=True,
        ocr_min_rec_score=0.0,
        ...
        **kwargs,
        ...
        self.clip_thr = clip_thr
        self.clip_thr_temperature = clip_thr_temperature
        self.return_ocr_transcript = bool(return_ocr_transcript)
        self.ocr_valid_only = bool(ocr_valid_only)
        self.ocr_min_rec_score = float(ocr_min_rec_score)
        self.ocr_transcript_separator = str(ocr_transcript_separator)
        self.default_prompt = "prompt"
    ...
    def _initialize_dataset(self, num_replicas, sort_dataset):
        ...
        cache_root = osp.expanduser(os.environ.get("WIDS_CACHE", "~/.cache/_wids_cache"))
        cache_dir = osp.join(cache_root, f"{getpass.getuser()}-{uuid}")
        if len(self.meta_paths) > 0:
            ...
            self.dataset = ShardListDataset(
                self.meta_path, cache_dir=cache_dir
            )
        self.ori_imgs_nums = len(self)
    ...
    def getdata(self, idx):
        ...
        if self.return_ocr_transcript:
            return sample + (
                _whole_image_ocr_transcript(
                    info,
                    valid_only=self.ocr_valid_only,
                    min_rec_score=self.ocr_min_rec_score,
                    separator=self.ocr_transcript_separator,
                ),
            )
        return sample
...
class PixDiTWebDatasetMS(PixDiTWebDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ratio_index = {}
        self.ratio_nums = {}
        self.interpolate_model = _pixel_interpolation(self.aspect_ratio)
...
class PixelDatasetMS(PixDiTWebDataset):
    ...
    def __init__(
        ...
        aspect_ratio_type: str = "ASPECT_RATIO_512",
        exclude_indices_path: Optional[str] = None,
        **kwargs,
    ):
        if "data_dir" in kwargs:
            data_dir = kwargs["data_dir"]
        ...
        self.excluded_source_indices = self._load_excluded_source_indices(exclude_indices_path)
        self.sample_indices: Optional[List[int]] = None
        if self.excluded_source_indices:
            self.sample_indices = [
                source_index
                for source_index in range(len(self.dataset))
                if source_index not in self.excluded_source_indices
            ]
            self.logger.info(
                f"Excluding {len(self.excluded_source_indices)} held-out samples; "
                f"training samples: {len(self.sample_indices)}/{len(self.dataset)}"
            )
    ...
    def _load_excluded_source_indices(self, exclude_indices_path: Optional[str]) -> set[int]:
        if not exclude_indices_path:
            return set()

        path = osp.expanduser(exclude_indices_path)
        if not osp.isfile(path):
            raise FileNotFoundError(f"Held-out index manifest not found: {path}")
        try:
            with open(path, encoding="utf-8") as stream:
                manifest = json.load(stream)
        except json.JSONDecodeError as error:
            raise ValueError(f"Held-out index manifest is not valid JSON: {path}") from error

        if not isinstance(manifest, dict) or not isinstance(manifest.get("exclude_indices"), list):
            raise ValueError(f"Held-out index manifest must contain an exclude_indices list: {path}")

        source_count = len(self.dataset)
        excluded: set[int] = set()
        for value in manifest["exclude_indices"]:
            if isinstance(value, bool) or not isinstance(value, int):
                raise ValueError(f"Held-out index is not an integer in {path}: {value!r}")
            if value < 0 or value >= source_count:
                raise ValueError(
                    f"Held-out index {value} is outside the source dataset range [0, {source_count})."
                )
            if value in excluded:
                raise ValueError(f"Duplicate held-out index in {path}: {value}")
            excluded.add(value)
        return excluded
    ...
    def __len__(self):
        # PixDiTWebDataset initializes ori_imgs_nums before this subclass builds
        # the filtered index, so retain the raw WIDS length during parent init.
        sample_indices = getattr(self, "sample_indices", None)
        return len(self.dataset) if sample_indices is None else len(sample_indices)

    def get_data_info(self, idx):
        return super().get_data_info(self._source_index(idx))
    ...
    def getdata(self, idx):
        data = self.dataset[self._source_index(idx)]
        info = data[".json"]
        self.key = data["__key__"]
        dataindex_info = {
            "index": data["__index__"],

...
class AugmentedDiTBlock(nn.Module):
    def __init__(
        ...
        use_promoe: bool = False,
        promoe_config: Optional[dict] = None,
    ):
        super().__init__()
        ...
        self.use_promoe = bool(use_promoe)
        if self.use_promoe:
            self.mlp = ProMoEFeedForward(hidden_size=hidden_size, **(promoe_config or {}))
        else:
    ...
    def forward(self, x, c, pos, mask=None, labels=None, return_aux_loss: bool = False):
        ...
        if self.use_promoe:
            if labels is None:
                raise ValueError("ProMoE patch blocks require class labels")
            mlp_output, rcl_loss = self.mlp(mlp_input, labels, return_aux_loss=return_aux_loss)
        else:
            mlp_output = self.mlp(mlp_input)
            rcl_loss = x.new_zeros(())
...
class PixDiT(nn.Module):
    def __init__(
        ...
        self.patch_blocks = nn.ModuleList(
            [self._make_patch_block(index) for index in range(self.patch_depth)]
        )
    ...
    def _make_patch_block(self, index: int) -> AugmentedDiTBlock:
        return AugmentedDiTBlock(self.hidden_size, self.num_groups)
    ...
    def forward(self, x, t, y, s=None, mask=None, return_aux_loss: bool = False):
        B, _, H, W = x.shape
        ...
        if s is None:
            ...
            total_rcl_loss = x.new_zeros(())
            for block in self.patch_blocks:
                if return_aux_loss:
                    s, rcl_loss = block(s, c, pos, mask, labels=y, return_aux_loss=True)
                    total_rcl_loss = total_rcl_loss + rcl_loss
                else:
                    s = block(s, c, pos, mask, labels=y)
        ...
        if return_aux_loss:
            return x_img, total_rcl_loss
        return x_img


class PixDiTProMoE(PixDiT):
    """RGB PixelDiT with ProMoE in alternating patch-level FFNs only."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.expert_intermediate_size = int(
            expert_intermediate_size if expert_intermediate_size is not None else 2 * hidden_size
        )
    ...
    def _make_patch_block(self, index: int) -> AugmentedDiTBlock:
        return AugmentedDiTBlock(
            self.hidden_size,
            self.num_groups,
            use_promoe=index % 2 == 1,
            promoe_config={
                "expert_intermediate_size": self.expert_intermediate_size,
                "num_routed_experts": self.num_routed_experts,
                "null_label": self.null_label,
                "rcl_temperature": self.rcl_temperature,
                "rcl_weight": self.rcl_weight,
            },
        )

    @property
    def num_promoe_patch_blocks(self) -> int:
        return sum(block.use_promoe for block in self.patch_blocks)

    def model_configuration(self) -> dict:
        total_parameters = sum(parameter.numel() for parameter in self.parameters())
        inactive_conditional_parameters = 0
        inactive_null_parameters = 0
        for block in self.patch_blocks:
            if not block.use_promoe:
                continue
            moe = block.mlp
            routed_parameters = sum(
                parameter.numel() for expert in moe.routed_experts for parameter in expert.parameters()
            )
            one_routed_parameters = sum(parameter.numel() for parameter in moe.routed_experts[0].parameters())
            unconditional_parameters = sum(parameter.numel() for parameter in moe.unconditional_expert.parameters())
            inactive_conditional_parameters += routed_parameters - one_routed_parameters + unconditional_parameters
            inactive_null_parameters += routed_parameters
        return {
            "architecture": "PixDiTProMoE",
            "patch_hidden_size": self.hidden_size,
            "patch_blocks": self.patch_depth,
            "promoe_patch_blocks": self.num_promoe_patch_blocks,
            "pit_blocks": self.pixel_depth,
            "routed_experts_per_promoe_block": self.num_routed_experts,
            "total_experts_per_promoe_block": self.num_routed_experts + 2,
            "shared_experts_per_promoe_block": 1,
            "unconditional_experts_per_promoe_block": 1,
            "expert_intermediate_size": self.expert_intermediate_size,
            "total_parameters": total_parameters,
            "activated_parameters_per_conditional_token": total_parameters - inactive_conditional_parameters,
            "activated_parameters_per_null_token": total_parameters - inactive_null_parameters,
        }

...
def resolve_checkpoint(path):
    ...
    if name == "pixeldit_t2i_v1.pth":
        local_path = Path(__file__).resolve().parents[1] / "ckpts" / "PixelDiT-1300M-1024px" / name
        if local_path.is_file():
            return str(local_path)
        raise FileNotFoundError(
            f"Missing local PixelDiT checkpoint: {local_path}. Run ckpts/download.sh first."
        )
    if name in KNOWN:
